...
import css from "./styles/AiPanel.module.less";
import clsx from "clsx";
import { PLANTS, AI_CATS } from "./constant";
import type { ModelItem, ModelStatus } from "./constant";
import { Ic, Btn, Badge } from "./ui";

type AiIntent = "create" | "optimize" | "status";
...
type ParsedCmd = ParsedCreate | ParsedOptimize | ParsedStatus;
...
type AiCardPayload = AiCardData | AiPlanData | AiOptData | AiStatusData;
...
export const AiPanel = ({
    ...
    setTimeout(() => {
      ...
      if (parsed.intent === "create") {
        ...
        const list = models.filter(
          (x) => x.plant === parsed.entities.plant,
        );
        addMsg({
          role: "ai",
  ...
  const OptCard = ({ data }: OptCardProps): React.ReactElement => {
    ...
    const [sel, setSel] = useState<number[]>(
      sugs.filter((s) => s.auto).map((s) => s.id),
    );
    const [st, setSt] = useState<"ready" | "launching" | "done">("ready");
    const m = data.model;
    const mc = {
        ...
        {msgs.map((m, i) => (
          ...
          >
            ...
            {m.card && m.card.type === "parse" && (
              <ParseCard parsed={(m.card as AiCardData).parsed} />
            )}
            {m.card && m.card.type === "plan" && (
              <PlanCard data={(m.card as AiPlanData).data} />
            )}
            {m.card && m.card.type === "opt" && (
              <OptCard data={(m.card as AiOptData).data} />
            )}
            {m.card && m.card.type === "status" && (
              <StatusCard data={(m.card as AiStatusData).data} />
            )}
          </div>
        ))}
        {phase && (
