...
import * as css from './helpCenter.modules.less';
import { sbGroups, sbItems, tourSteps } from './constant';
...
export default function HelpCenter() {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      if (wn && !wn.contains(e.target as Node)) {
        setWhatsNewOpen(false);
      }
    };
    ...
    <div>
      ...
      <div className="demo-bar">
        ...
        <div className="demo-tabs">
          <button className={`demo-tab${activeView === 'hc' ? ' active' : ''}`} onClick={() => switchView('hc')}>
            📖 帮助中心(独立)
          </button>
          <button className={`demo-tab${activeView === 'sys' ? ' active' : ''}`} onClick={() => switchView('sys')}>
            🖥 系统内集成 ?抽屉
          </button>
        </div>
          ...
          <div className="hc-body">
                  ...
                  <div className={`sb-items${openGroups.has(g.id) ? ' open' : ''}`}>
                    {sbItems[g.id].map((item, idx) => (
                      <div
                        ...
                        style={{
                          opacity:
                            searchQuery.length > 1 && !item.text.toLowerCase().includes(searchQuery.toLowerCase())
                              ? 0.3
                              : 1,
                        }}
            ...
            <main className="hc-main">
              ...
              <header className="article-head">
                ...
                <div className="meta-row">
                  <span className="meta-tag role">
                    <span className="dot"></span>专业工程师
                  </span>
              ...
              <div className="article-body">
                <p className="lede">
                  读完本文,您将能为同一台设备配置「运行」「并网」「停机」等多个工况模式,并理解模式的两个核心作用 ——
                  限定样本选取范围、控制 AI 计算的生效时段。
                </p>

                <h2 id="s1">什么是「模式」</h2>
                <p>
                  模式(Mode)是平台中用于<strong>判定当前是否属于某种运行工况</strong>
                  的一组表达式。不同模式以不同颜色的矩形框区分,在模型配置页中部展示。模式有两个核心作用:
                </p>
                <ul>
                  <li>
                    <strong>限定样本选取范围</strong> —— 选取训练样本时,只有落在模式条件成立的时段内的数据才会被使用;
                  </li>
                  <li>
                    <strong>控制 AI 计算的生效时段</strong> —— 模型上线后,只有在模式条件满足时,系统才会执行 AI
                    计算和预测。设备停机后模式不再满足,AI 计算会自动停止,<strong>无需手动下线模型</strong>。
                  </li>
                </ul>
                ...
                <ol className="steps">
                  <li>
                    进入 <code>建模平台 → 配置中心 → 模型</code>,选择目标模型,在中部模式配置区点击「
                    <strong>新增模式</strong>」。
                  </li>
                  <li>
                    填写<strong>模式名称</strong>(建议简洁状态描述,如「运行」「并网」),设置<strong>优先级</strong>
                    (1–5,数字越小优先级越高),选择<strong>标识色</strong>。
                  </li>
                  <li>
                    点击加号定义模式表达式:选择测点类型 → 选择具体测点 → 选择运算符(≥/≤/&gt;/&lt;/=/≠ 共 6 种)→
                    填入阈值。
                  </li>
                  <li>
                    需要多条件时继续添加,并选择<strong>逻辑关系 AND / OR</strong>
                    ;系统严格按从上到下顺序判断,关键条件放前面。
                  </li>
                  <li>
                    点击保存。也可以选择<strong>导入全局模式</strong>(多个模型共用相同工况时,可一键导入)。
                  </li>
                </ol>
                ...
                <div className="callout callout-warn">
                  <div>
                    <strong>注意</strong>
                    <strong>AND 和 OR 之间没有优先级</strong>,系统严格按从上到下顺序依次判断。如果配置{' '}
                    <code>A AND B OR C</code>,实际执行的是 <code>(A AND B) OR C</code>,而不是{' '}
                    <code>A AND (B OR C)</code>。请把关键条件放在最前。
                  </div>
                </div>
                  ...
                  <div className="video-chapters">
                    ...
                    <div className="chapter-list">
                      <div className="chapter">
                        ...
                        <span>单元一 · 创建模型与基础配置</span>
                      </div>
                      <div className="chapter active">
                        <span className="ts">02:30</span>
                        <span>单元二 · 配置工况模式 ◀ 当前</span>
                      </div>
                      <div className="chapter">
                        <span className="ts">05:00</span>
                        <span>单元三 · 测点参数配置</span>
                      </div>
                      <div className="chapter">
                        <span className="ts">07:30</span>
                        <span>单元四 · 样本选取与模型训练</span>
                      </div>
                      <div className="chapter">
                        <span className="ts">10:30</span>
                        <span>单元五 · 多工况模型</span>
                      </div>
                    </div>
                  </div>
                ...
                <h2 id="s3">优先级的含义</h2>
                <p>
                  当<strong>多个模式条件同时满足</strong>(也就是判断规则出现重叠)时,系统会选择
                  <strong>优先级最高的模式</strong>生效。建议:
                </p>
                <ul>
                  <li>
                    把最精确、最严格的工况设置为<strong>高优先级(数字小)</strong>;
                  </li>
                ...
                <div className="callout callout-info">
                  ...
                  <div>
                    <strong>建议</strong>
                    如果您正在为<strong>多工况模型</strong>配置模式,请注意:每个工况下都需要单独训练,只有
                    <strong>所有工况都通过训练</strong>之后,多工况模型的【上线】按钮才会变为可用。
                  </div>
                </div>

                <h2 id="s4">在主系统中打开此功能</h2>
                <p>点击下方按钮可在新页签中直接跳到「模型配置 → 工况模式」页:</p>
                <div className={css.btnMarginWrap}>
                  <a
                    className="btn btn-secondary"
                    onClick={() => showSnack('已跳转到 /modeling/config/model/mode(原型演示)')}>
                    ...
                    {[
                      {
                        meta: '模型配置 · 文档',
                        title: '如何选取训练样本并训练模型',
                        sub: '样本必须代表正常工况,单模型总有效点 ≤ 5 万',
                      },
                      {
                        meta: '模型配置 · 文档',
                        title: '如何配置测点参数',
                        sub: 'IO 类型(输入/输出/输入输出)、残差阈值、延迟策略',
                      },
                      {
                        meta: '规则配置 · 文档',
                        title: '规则模型中如何导入全局模式',
                        sub: '规则模型与诊断模型可共用全局模式',
                      },
                      {
                        meta: 'FAQ · 常见',
                        title: '为什么我的模式频繁切换?',
                        sub: '检查判别条件中是否需要加入滞回缓冲',
                      },
                    ].map((c, i) => (
                ...
                <div className="feedback-bar">
                  ...
                  <button className="bad" onClick={() => showSnack('感谢您的反馈,我们会优化此文档')}>
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <path d="M17 14V2M9 18.12 10 14H4.17a2 2 0 0 1-1.92-2.56l2.33-8A2 2 0 0 1 6.5 2H17" />
                    </svg>
                    没用
                  </button>
                </div>
            ...
            <aside className="hc-toc">
              ...
              <div className="toc-side-card">
                ...
                <div className={`${css.mono} ${css.tocRouteMono}`}>
                  /modeling/config
                  <br />
                  /modeling/config/model
                  <br />
      ...
      <div className={`view${activeView === 'sys' ? ' active' : ''}`}>
        <div className="sys-shell">
              ...
              <div id="whatsNew" className={`whats-new${whatsNewOpen ? ' open' : ''}`}>
                ...
                <div className="wn-list">
                  {[
                    ...
                    {
                      title: '告警单 → 案例库自动归档',
                      desc: '关闭告警单时勾选「更新至案例库」,自动沉淀为可复用知识。',
                    },
                  ].map((item, i) => (
          ...
          <div className={css.subNav}>
            <span className={css.subNavLabel}>
              配置中心 ›
            </span>
            <a className={css.subNavActive}>
              模型
            </a>
            <a className={css.subNavLink}>
              规则模型
            </a>
            <a className={css.subNavLink}>
              计算测点
            ...
            <span className={css.subNavSuffix}>
              左侧目录树:
              <span className={`${css.mono} ${css.subNavTreeText}`}>
          ...
          <div className="sys-work">
            ...
            <div className="work-grid">
              <aside className="work-side">
                ...
                <div className="step-nav">
                  <a className="done">
                    <span className="num">✓</span> 基本信息
                  </a>
                  <a className="done">
                    <span className="num">✓</span> 测点选取
                  </a>
                  <a className="active">
                    <span className="num">3</span> 工况模式
                  </a>
                  <a>
                    <span className="num">4</span> 测点参数
                  </a>
                  <a>
                    <span className="num">5</span> 样本选取
                  </a>
                  <a>
                    <span className="num">6</span> 模型训练
                  </a>
                </div>
              </aside>
              ...
              <section className="work-card">
                ...
                <div className="form-row">
                  <div className="form-label">
                    ...
                    <span className="info-i" tabIndex={0} title="建议使用简洁的状态描述">
                      i
                    </span>
                  </div>
                  ...
                  <div className="form-label">
                    ...
                    <span className="info-i has-popover" tabIndex={0}>
                      ...
                      <div className="popover">
                        ...
                        <p>
                          取值范围 1–5,<strong>数字越小优先级越高</strong>
                          。当多个模式的判断规则出现重叠(条件同时满足)时,系统会选择优先级最高的模式生效。
                        </p>
                ...
                <div className="form-row">
                  ...
                  <div className={`form-control ${css.formControlFull}`}>
                    <div className={css.colorSwatchRow}>
                      {[
                        ...
                        <span
                          key={i}
                          className={`${css.colorSwatch}${c.selected ? ` ${css.colorSwatchSelected}` : ''}`}
                          style={{ background: c.bg, boxShadow: c.selected ? `0 0 0 2px ${c.bg}` : undefined }}></span>
                      ))}
                    </div>
                  ...
                  <div className="form-label">
                    ...
                    <span className="info-i has-popover" tabIndex={0}>
                      ...
                      <div className="popover">
                        ...
                        <p>
                          <strong>AND 和 OR 之间没有优先级</strong>
                          ,系统严格按从上到下顺序依次判断,因此要将关键条件放在前面。
                        </p>
                        <div className="pop-img">
                          如 A AND B OR C<br />
                          实际执行 = (A AND B) OR C
                  ...
                  <div className={`form-control ${css.formControlFull}`}>
                    <div className={css.exprCol}>
                      <div className={css.exprRow}>
                        ...
                        <span className={`${css.kbdPill} ${css.kbdPillWhite}`}>
                          主轴转速 SHAFT_SPEED
                        </span>
                        <span className={`${css.kbdPill} ${css.kbdPillDark}`}>
                          ≥
                        </span>
                        <span className={`${css.kbdPill} ${css.kbdPillWhite}`}>
                          3000
                        </span>
                        <span className={`${css.kbdPill} ${css.kbdPillUnit}`}>
                          rpm
                        </span>
                      </div>
                      <div className={css.exprAndRow}>
                        <span className={`${css.kbdPill} ${css.kbdPillAnd}`}>
                          AND
                        </span>
                      ...
                      <button className={css.addExprBtn}>
                        + 添加表达式
                      </button>
                    </div>
                ...
                <div className="form-row">
                  ...
                  <div className={`form-control ${css.formControlFull}`}>
                    <div className={css.modeTagRow}>
                      <span className={`meta-tag ${css.modeTagSteel}`}>
                        <span className="dot"></span>运行 · 优先级 2(本次编辑)
                      </span>
                      <span className={`meta-tag ${css.modeTagAmber}`}>
                        <span className="dot"></span>并网 · 优先级 1
                      </span>
                      <span className={`meta-tag ${css.modeTagAdd}`}>
                        + 新增模式
                      </span>
                      <span className={`meta-tag ${css.modeTagImport}`}>
                        ⤵ 导入全局模式
                      </span>
                    </div>
            ...
            <div className={css.sysTip}>
              💡 提示:点击页面右上角 <span className={css.kbdPill}>? 本页帮助</span> 可在不离开本页的情况下查看相关教程;按{' '}
              <span className={css.kbdPill}>Shift + /</span> 也可快速打开。
              <a
                className={css.sysTipLink}
                onClick={() => {
                  setTourIndex(0);
                  setTourOpen(true);
                }}>
      ...
      <aside className={`drawer${drawerOpen ? ' open' : ''}`}>
        <div className="drawer-head">
          <div className="dh-top">
            ...
            <span className="dh-route">
              {drawerType === 'global' ? '全平台 (M1 模式)' : 'routeKey: /modeling/config/model/mode'}
            </span>
            <div className="dh-actions">
              <button
                className={`dh-icon-btn${pinned ? ' pinned' : ''}`}
                title="钉住"
                onClick={() => setPinned(v => !v)}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M12 17.5V22M9 4h6l-1 7 4 3v2H6v-2l4-3-1-7z" />
                </svg>
              </button>
          ...
          <div className="drawer-tabs">
            {(['doc', 'video', 'faq'] as const).map(tab => (
              <button
                key={tab}
                className={`drawer-tab${activeDrawerTab === tab ? ' active' : ''}`}
                onClick={() => setActiveDrawerTab(tab)}>
                {tab === 'doc' && (
                  <>
                    📄 文档 <span className="cnt">4</span>
                  </>
                )}
                {tab === 'video' && (
                  <>
                    🎬 视频 <span className="cnt">1</span>
                  </>
                )}
                {tab === 'faq' && (
                  <>
                    ❓ FAQ <span className="cnt">3</span>
                  </>
                )}
              </button>
            ))}
        ...
        <div className="drawer-body">
          {/* DOC tab */}
          <div className={activeDrawerTab === 'doc' ? css.drawerTabPanelVisible : css.drawerTabPanel}>
              ...
              {[
                {
                  ...
                  time: '4 min',
                  title: '如何配置工况模式',
                  sub: '优先级 1–5、6 种运算符、AND/OR 从上到下顺序、导入全局模式',
                  msg: '已打开《如何配置工况模式》',
                },
                {
                  role: '工程师',
                  cat: '模型配置',
                  time: '3 min',
                  title: '如何创建一个模型(基本信息与算法选择)',
                  sub: '三种算法对比:MMLA 机理 / NRDA 时序关联 / PBA 非时序',
                  msg: '已打开《如何创建一个模型》',
                },
                {
                  role: '工程师',
                  ...
                  time: '5 min',
                  title: '如何配置测点参数',
                  sub: 'IO 类型(输入/输出/输入输出/不使用)、残差告警阈值、3 种延迟策略',
                  msg: '已打开《如何配置测点参数》',
                },
                {
                  role: '工程师',
                  ...
                  time: '4 min',
                  title: '如何选取训练样本并训练模型',
                  sub: '样本须代表正常工况,单模型有效点 ≤ 5 万,三色曲线解读',
                  msg: '已打开《如何选取训练样本并训练模型》',
                },
              ].map((doc, i) => (
            ...
            <div className={`recommended-label ${css.recommendedLabelSpacing}`}>
              📺 推荐视频
            </div>
            <div className="help-video-card" onClick={() => showSnack('▶ 自动跳到 V1 · 02:30 单元二 — 配置工况模式')}>
              <div className="hv-poster">
                <div className={`play-btn ${css.playBtnSmall}`}>
          ...
          {/* VIDEO tab */}
          <div className={activeDrawerTab === 'video' ? css.drawerTabPanelVisible : css.drawerTabPanel}>
            ...
            <div className={css.drawerVideoBg}>
              <div className={css.drawerChLabel}>
                单元章节(点击跳转)
              </div>
              <div className="chapter-list">
                {[
                  ...
                  <div key={i} className={`chapter${active ? ' active' : ''}`}>
                    <span className="ts">{ts}</span>
                    <span>{label}</span>
                  </div>
                ))}
              </div>
              <div className={css.drawerChLabelSpacing}>
                单元二 子章节(6 画面)
              </div>
              <div className="chapter-list">
                {[
          ...
          {/* FAQ tab */}
          <div className={activeDrawerTab === 'faq' ? css.drawerTabPanelVisible : css.drawerTabPanel}>
            <div className="recommended-label">高频问题</div>
            <div className="help-doc-list">
              {[
                {
                  title: '为什么我的模式频繁切换?',
                  sub: '检查表达式中是否需要加入滞回(hysteresis)缓冲,建议留 3–5% 死区',
                },
                {
                  title: 'AND 在 OR 之后是不是优先级更高?',
                  sub: '不是。AND/OR 没有优先级之分,系统严格按从上到下顺序判断,把关键条件放最前',
                },
                {
                  title: '我没创建任何模式,模型能上线吗?',
                  sub: '不能。模式控制 AI 计算的生效时段,缺失模式会导致模型上线后不执行任何计算',
                },
              ].map((faq, i) => (
      ...
      <div className={`tour-overlay${tourOpen ? ' open' : ''}`}>
        <div className="tour-card">
          ...
          <div className="tour-actions">
            <button className="btn btn-secondary" onClick={() => setTourOpen(false)}>
              跳过
            </button>
            <div className="tour-spacer"></div>
            <div className="tour-dots">
              {tourSteps.map((_, i) => (
                <i key={i} className={i === tourIndex ? 'active' : ''}></i>
              ))}
            </div>
